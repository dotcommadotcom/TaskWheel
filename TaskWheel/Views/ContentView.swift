...
struct ContentView: View {
    ...
    var body: some View {
        VStack {
            Text("You selected: \(taskViewModel.currentTitle())")
            
            Picker("Choose a list", selection: $selected) {
                ForEach(taskViewModel.taskLists) {
                    Text($0.title)
                }
            }
            .pickerStyle(.menu)
...
struct ValuePicker<SelectionValue: Hashable, Content: View>: View {
